...
use Piwik\Plugins\LoginRolesLdap\Ldap\Client;
use Piwik\Plugins\LoginRolesLdap\Ldap\ServerInfo;
use Psr\Log\LoggerInterface;

/**
 * Utility class with methods to manage LoginRolesLdap INI configuration.
 */
class Config
{
    ...
    /**
     * Returns an INI option value that is stored in the `[LoginRolesLdap]` config section.
     *
    ...
    {
        return self::getConfigOptionFrom(PiwikConfig::getInstance()->LoginRolesLdap, $optionName);
    }
    ...
    {
        $configName = 'LoginRolesLdap_' . $server;
        ...
        if (empty($serverNameList)) {
            ...
            foreach ($serverNameList as $name) {
                try {
                    ...
                    $logger->debug("LoginRolesLdap\\Config::{func}: LDAP server info '{name}' is configured incorrectly: {message}", array(
                        'func' => __FUNCTION__,
                        'name' => $name,
    ...
    {
        $loginRolesLdap = PiwikConfig::getInstance()->LoginRolesLdap;

        foreach (self::$defaultConfig as $name => $value) {
            if (isset($config[$name])) {
                $loginRolesLdap[$name] = $config[$name];
            }
        }

        PiwikConfig::getInstance()->LoginRolesLdap = $loginRolesLdap;
    ...
    public static function saveLdapServerConfigs($servers)
    {
        ...
        PiwikConfig::getInstance()->LoginRolesLdap['servers']= $serverNames;
