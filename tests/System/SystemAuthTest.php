 ...
 */
namespace Piwik\Plugins\LoginRolesLdap\tests\System;
...
use Piwik\Date;
use Piwik\Db;
use Piwik\Plugins\LoginRolesLdap\LoginRolesLdap;
use Piwik\Plugins\LoginRolesLdap\Auth\LdapAuth;
use Piwik\Plugins\LoginRolesLdap\Auth\SynchronizedAuth;
use Piwik\Plugins\LoginRolesLdap\Auth\WebServerAuth;
use Piwik\Plugins\LoginRolesLdap\tests\Integration\LdapIntegrationTest;
use Piwik\Tests\Framework\Fixture;
use Piwik\Tests\Framework\TestingEnvironmentVariables;

/**
 * @group LoginRolesLdap
 * @group LoginRolesLdap_System
 * @group LoginRolesLdap_SystemAuthTest
 */
class SystemAuthTest extends LdapIntegrationTest
{
    ...
    private function setUpLdap($authStrategy)
    {
        ...
        if ($authStrategy == 'ldap_only') {
            Config::getInstance()->LoginRolesLdap['use_ldap_for_authentication'] = 1;
            $configOverride['LoginRolesLdap']['use_ldap_for_authentication'] = 1;
        } else if ($authStrategy == 'synchronized') {
            Config::getInstance()->LoginRolesLdap['use_ldap_for_authentication'] = 0;
            $configOverride['LoginRolesLdap']['use_ldap_for_authentication'] = 0;
        } else if ($authStrategy == 'web_server') {
            Config::getInstance()->LoginRolesLdap['use_webserver_auth'] = 1;
            $configOverride['LoginRolesLdap']['use_webserver_auth'] = 1;
        } else {
     ...
     * @expectedExceptionMessage LoginRolesLdap_LdapUserCantChangePassword
     */
    public function testLdapUserPassChange()
    {
        ...
        $loginRolesLdap = new LoginRolesLdap();
        $loginRolesLdap->disablePasswordChangeForLdapUsers($auth);
    }
}
